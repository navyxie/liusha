...
LIUSHA.prototype.flowTopUp = function(params,cb){
	...
	basePostQuest(url,toParams,function(err,data){
		if(!err && _.isObject(data) && data.code == 0 && _.isObject(data.data) && data.data.orderId){
			cb(null,data);
		}else{
			console.error('流沙流量充值失败',data);
			cb(err || '流沙流量充值失败',data);
		}		
	});
}
...
LIUSHA.prototype.exists = function(orderId,cb){
...
LIUSHA.prototype.success = function(orderId,status,cb){
	...
	this.query(orderId,function(err,data){
		if(!err && _.isObject(data)){
			//status	订单的充值状态，0：充值失败，1：正在充值，2：充值成功
			if(data.code == 0 && _.isObject(data.data) && data.data.orderId && data.data.status == status){
				cb(null,true);
			}else{	
				cb(null,false);
			}
